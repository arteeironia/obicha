...
export default function LandingClient({ products, socialPosts, pinterestPins, siteConfig }: Props) {
  const lojaUrl = siteConfig.loja_url || '{lojaUrl}'
  const instagramUrl = siteConfig.instagram_url || '{instagramUrl}'
  const tiktokUrl = siteConfig.tiktok_url || 'https://www.tiktok.com/@obicha_camisetas'
  const pinterestUrl = siteConfig.pinterest_url || '{pinterestUrl}'
  const whatsappUrl = siteConfig.whatsapp_url || '{whatsappUrl}'
    ...
    <>
      ...
      <aside className={`sidebar ${mobileMenuOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          <a href="#manifesto" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><path d="M3 5h14M3 9h10M3 13h12M3 17h8"/></svg>
            </span>
            Manifesto
          </a>
          <a href="#produtos" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><path d="M4 7l2-3h8l2 3"/><path d="M3 7h14v10H3z"/><path d="M8 7v2a2 2 0 004 0V7"/></svg>
            </span>
            Produtos
          </a>
          <a href="#compromissos" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><circle cx="10" cy="10" r="7"/><path d="M7 10l2 2 4-4"/></svg>
            </span>
            Missão
          </a>
          <a href="#amargen" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><path d="M10 17S3 12.5 3 7.5A4 4 0 0110 5a4 4 0 017 2.5C17 12.5 10 17 10 17z"/></svg>
            </span>
            Causa
          </a>
          <a href="#social" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><circle cx="5" cy="10" r="2"/><circle cx="15" cy="5" r="2"/><circle cx="15" cy="15" r="2"/><path d="M7 9l6-3M7 11l6 3"/></svg>
            </span>
            Redes
          </a>
          <a href="/blog" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><path d="M4 4h12v2H4zM4 8h8v2H4zM4 12h10v2H4zM4 16h6v2H4z"/></svg>
            </span>
            Blog
          </a>
          <a href="/parcerias" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 15a4 4 0 00-8 0v1h8v-1zM18 17v-1a3 3 0 00-2-2.83M4 14.17A3 3 0 002 17v1"/></svg>
            </span>
            Parcerias
          </a>
          <a href="/projeto-social" className="sidebar-link" onClick={() => setMobileMenuOpen(false)}>
            <span className="sl-icon">
              <svg viewBox="0 0 20 20"><path d="M10 17S3 12.5 3 7.5A4 4 0 0110 5a4 4 0 017 2.5C17 12.5 10 17 10 17z"/><path d="M10 9v4M8 11h4"/></svg>
            </span>
            Projeto Social
          </a>
        </nav>
        ...
        <div className="sidebar-bottom">
          <a href={lojaUrl} target="_blank" className="sidebar-cta">
            Entrar na Loja
          </a>
        </div>
      </aside>
      ...
      <section id="hero" style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden', background:'var(--navy)' }}>
        ...
        <div className="hero-content" style={{ position:'relative', zIndex:2, textAlign:'center', padding:'2rem' }}>
          ...
          <img src="/banner.png" alt="Ô bicha! — Deboche, Amor e Resistência" className="hero-banner-img" style={{ maxWidth:'min(680px, 88vw)', width:'100%', borderRadius:4, animation:'banner-glow 3s ease-in-out infinite alternate', marginBottom:'2rem' }} />
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:'.5rem', marginBottom:'3rem', flexWrap:'wrap' }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              ...
              {cat === 'todos' ? 'Todos' : catLabel(cat) + 's'}
            </button>
          ))}
        </div>
      ...
      <footer style={{ background:'#0f1a2e', borderTop:'1px solid rgba(212,168,67,.2)', padding:'3rem 2rem', textAlign:'center' }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:'2rem', flexWrap:'wrap', marginBottom:'2rem' }}>
          ...
          <a href={lojaUrl} target="_blank"
            ...
            onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.5)'}
          >Loja</a>
          <a href="https://institutoamargen.com.br" target="_blank"
            style={{ color:'rgba(242,235,217,.5)', textDecoration:'none', fontSize:'.8rem', letterSpacing:'1px', textTransform:'uppercase', transition:'color .3s' }}
            onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
            onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.5)'}
          >Instituto Amargen</a>
        ...
        <p style={{ fontSize:'.72rem', color:'rgba(242,235,217,.2)', marginBottom:'.8rem' }}>
          Fernando Carsi · CPF: 037.028.966-86 · Campinas/SP · Não temos loja física
        </p>
        <div style={{ display:'flex', justifyContent:'center', gap:'1.5rem', flexWrap:'wrap' }}>
          <a href="mailto:falenosco@obicha.com.br" style={{ fontSize:'.72rem', color:'rgba(242,235,217,.3)', textDecoration:'none', transition:'color .3s' }}
            onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
            onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.3)'}
          >Fale Conosco</a>
          <span style={{ color:'rgba(242,235,217,.15)' }}>·</span>
          <a href="https://umapenca.com/obicha/central-de-ajuda/#reamaze#0#/kb/trocas-e-devolucoes/" target="_blank" style={{ fontSize:'.72rem', color:'rgba(242,235,217,.3)', textDecoration:'none', transition:'color .3s' }}
            onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
            onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.3)'}
          >Trocas e Devoluções</a>
          <span style={{ color:'rgba(242,235,217,.15)' }}>·</span>
          <a href="/parcerias" style={{ fontSize:'.72rem', color:'rgba(242,235,217,.3)', textDecoration:'none', transition:'color .3s' }}
            onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
            onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.3)'}
          >Parcerias</a>
          <span style={{ color:'rgba(242,235,217,.15)' }}>·</span>
          <a href="/projeto-social" style={{ fontSize:'.72rem', color:'rgba(242,235,217,.3)', textDecoration:'none', transition:'color .3s' }}
            onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
            onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.3)'}
          >Projeto Social</a>
        </div>
