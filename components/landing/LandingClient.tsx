...
export default function LandingClient({ products, socialPosts, pinterestPins, siteConfig, highlights }: Props) {
  ...
  // Flatten all highlight images into slides
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .carousel-slide-info { padding:1rem 1.2rem; background:rgba(15,26,46,.95); }
        .carousel-slide-title { font-family:var(--font-playfair); font-size:1rem; font-weight:700; color:var(--creme); margin:0 0 .3rem; }
        .carousel-slide-label { font-family:var(--font-bebas); font-size:.7rem; letter-spacing:3px; color:var(--gold); opacity:.8; }
        .carousel-slide-price { display:flex; align-items:center; gap:.8rem; margin-top:.4rem; }
        .carousel-slide-price-old { font-size:.8rem; color:rgba(242,235,217,.4); text-decoration:line-through; }
        .carousel-slide-price-new { font-family:var(--font-bebas); font-size:1.1rem; color:var(--red); letter-spacing:1px; }
        .carousel-slide-btn { display:inline-block; margin-top:.8rem; padding:.5rem 1.2rem; background:var(--red); color:var(--creme); font-family:var(--font-bebas); letter-spacing:2px; font-size:.8rem; text-decoration:none; transition:background .3s; }
        .carousel-slide-btn:hover { background:var(--gold); color:var(--navy); }

        .carousel-btn { position:absolute; top:calc(50% - 30px); transform:translateY(-50%); background:rgba(26,39,68,.8); border:1px solid rgba(212,168,67,.4); color:var(--gold); width:36px; height:36px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:1.1rem; transition:all .3s; z-index:2; }
        .carousel-btn:hover { background:var(--gold); color:var(--navy); }
        .carousel-btn.prev { left:.6rem; }
        .carousel-btn.next { right:.6rem; }
        .carousel-dots { display:flex; justify-content:center; gap:.4rem; padding:.6rem 0; background:rgba(15,26,46,.95); }
        .carousel-dot { width:6px; height:6px; border-radius:50%; background:rgba(242,235,217,.25); border:none; cursor:pointer; transition:all .3s; padding:0; }
        .carousel-dot.active { background:var(--gold); transform:scale(1.3); }

        /* Grid de destaques */
        .highlights-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(200px,1fr)); gap:1rem; max-width:1200px; margin:0 auto; }
        .highlight-card { background:rgba(255,255,255,.03); border:1px solid rgba(212,168,67,.15); border-radius:4px; overflow:hidden; transition:all .4s; text-decoration:none; display:block; }
        .highlight-card:hover { border-color:var(--gold); transform:translateY(-4px); box-shadow:0 12px 30px rgba(0,0,0,.4); }
        .highlight-card img { width:100%; aspect-ratio:1; object-fit:cover; display:block; }
        .highlight-card-info { padding:.8rem 1rem; }
        .highlight-card-label { font-family:var(--font-bebas); font-size:.65rem; letter-spacing:3px; color:var(--gold); opacity:.7; margin-bottom:.2rem; }
        .highlight-card-title { font-family:var(--font-playfair); font-size:.9rem; font-weight:700; color:var(--creme); line-height:1.3; }
        .highlight-card-prices { display:flex; align-items:center; gap:.6rem; margin-top:.4rem; }
        .highlight-price-old { font-size:.75rem; color:rgba(242,235,217,.4); text-decoration:line-through; }
        .highlight-price-new { font-family:var(--font-bebas); font-size:.95rem; color:var(--red); }
        ...
        @media(max-width:600px) {
          ...
          .amargen-grid { grid-template-columns:1fr!important; }
          .highlights-grid { grid-template-columns:repeat(2, 1fr); gap:.75rem; }
        }
      `}</style>
      ...
      <aside className={`sidebar ${mobileMenuOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          {[
            ['#manifesto','Manifesto','M3 5h14M3 9h10M3 13h12M3 17h8'],
            ['#produtos','Produtos','M4 7l2-3h8l2 3 M3 7h14v10H3z M8 7v2a2 2 0 004 0V7'],
            ['#compromissos','Missão','circle:10,10,7 M7 10l2 2 4-4'],
            ['#amargen','Causa','M10 17S3 12.5 3 7.5A4 4 0 0110 5a4 4 0 017 2.5C17 12.5 10 17 10 17z'],
            ['#social','Redes','circle:5,10,2 circle:15,5,2 circle:15,15,2 M7 9l6-3M7 11l6 3'],
          ].map(([href, label]) => (
      ...
      <div className="main-content">
        ...
        <section id="manifesto" style={{ background:'var(--creme)', color:'var(--navy)', padding:'6rem 2rem', overflow:'hidden', position:'relative' }}>
          <div style={{ maxWidth:900, margin:'0 auto' }}>
            ...
            <div style={{ fontSize:'1.05rem', lineHeight:1.9, color:'#2a2a2a', maxWidth:700 }}>
              <p>A Ô bicha! não nasceu para passar despercebida. Criamos <strong>manifestos portáteis.</strong></p>
              <p style={{ marginTop:'1.2rem' }}>Em algodão, estonada, dry fit ou modal tech — sempre com estampa que tem algo a dizer.</p>
            </div>
          </div>
        </section>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:'2rem', maxWidth:1200, margin:'0 auto' }}>
            {filteredProducts.length === 0 ? (
              ...
              <div key={p.id} className="product-card">
                {p.image_url
                  ? <div className="product-card-img-wrap" onClick={() => setLightboxImg(p.image_url!)}>
        ...
        <footer style={{ background:'#0f1a2e', borderTop:'1px solid rgba(212,168,67,.2)', padding:'3rem 2rem', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', justifyContent:'center', gap:'1.5rem', marginBottom:'2rem' }}>
            <a href={instagramUrl} target="_blank" className="social-icon-link" title="Instagram">
            ...
            <a href={tiktokUrl} target="_blank" className="social-icon-link" title="TikTok">
            ...
            <a href={pinterestUrl} target="_blank" className="social-icon-link" title="Pinterest">
            ...
            <a href={whatsappUrl} target="_blank" className="social-icon-link" title="WhatsApp">
          ...
          <div style={{ display:'flex', justifyContent:'center', gap:'2rem', flexWrap:'wrap', marginBottom:'2rem' }}>
            ...
            <a href={lojaUrl} target="_blank" style={{ color:'rgba(242,235,217,.5)', textDecoration:'none', fontSize:'.8rem', letterSpacing:'1px', textTransform:'uppercase', transition:'color .3s' }}
              onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
              onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.5)'}
            >Loja</a>
            <a href="https://institutoamargen.com.br" target="_blank" style={{ color:'rgba(242,235,217,.5)', textDecoration:'none', fontSize:'.8rem', letterSpacing:'1px', textTransform:'uppercase', transition:'color .3s' }}
              onMouseEnter={e => (e.target as HTMLElement).style.color='var(--gold)'}
              onMouseLeave={e => (e.target as HTMLElement).style.color='rgba(242,235,217,.5)'}
            >Instituto Amargen</a>
          </div>
